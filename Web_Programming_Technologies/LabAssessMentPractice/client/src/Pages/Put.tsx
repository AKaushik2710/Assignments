...
export default function Post(){
    ...
    async function handlePut(){
        await axios.put(`http://localhost:3000/${idRef.current!.value}`, {
            title : titleRef.current!.value,
            author : authorRef.current!.value,
            pub : pubRef.current!.value,
            genre : genreRef.current!.value,
            avail : availRef.current!.value
        });
    }
    ...
    return <div className="container-fluid vh-100">
        <div className="row w-100 h-100">
            ...
            <div className="col-6 h-100">
                ...
                <button className="btn btn-primary" onClick={() =>handlePut().then(()=> navigate('/'))}>Put</button>
            </div>
        </div>
    </div>
}